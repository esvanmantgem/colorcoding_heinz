...
namespace colcod {
    ...
    public:
        ...
        inline void process_leave(MwcsPreGraphType &parser, int nr_of_colors, int nr_of_iterations, Node orgC, CcResult &bestResult, NodeSet nodesInBlock) {
            ...
            if (nBlocks != 0) {
                for (int blockDegree = nBlocks > 1 ? 1 : 0; blockDegree >= 0; --blockDegree) {
                    ...
                    while (!leaves.empty()) {
                        ...
                        process_leave(parser, nr_of_colors, nr_of_iterations, orgC, bestResult, nodesInBlock);
        ...
        void solve(MwcsPreGraphType &parser, std::string ccType, int nr_of_colors, int nr_of_iterations, NodeSet rootNodeSet) {
            std::vector<double> nodes;
            std::vector<std::string> labels;
            std::vector<std::vector<int>> neighbors;
