...
#include "colorcoding/CcAdapter.h"
...
// Eline
typedef colcod::CcAdapter<Graph> CcAdapterType;
...
int main(int argc, char** argv)
{
    ...
    ap
            .boolOption("version", "Show version number")
                    //Eline cc
            .refOption("cc", "Type of color coding", ccType, false)
            .refOption("col", "Number of colors", nrOfColors, false)
            .refOption("it", "Number of iterations", nrOfIterations, false)
    ...
    // Eline
    bool colcod = ap.given("cc");
    if (colcod)
    {
        if(!(ccType.compare("rooted") == 0 || ccType.compare("unrooted") == 0 || ccType.compare("blockcut") == 0)) {
            std::cerr << "Invalid type of color coding: rooted, unrooted, blockcut" << std::endl;
            return 1;
        }
    ...
    // Eline: added last of or, test
    MwcsGraphType* pMwcs;
    MwcsPreprocessedGraphType* pPreprocessedMwcs = NULL;
    if (!noPreprocess || ap.given("cc"))
    {
    ...
    // Eline: added last part to prevent preprocessing in case of rooted with colcod
    if (pPreprocessedMwcs && (noEnum || rootNodeSet.size() > 0) && !ap.given("cc"))
    {
    ...
    try {
        ...
        if (ap.given("cc")) {
            CcAdapterType *ccadapter = new CcAdapterType();
            ccadapter->solve(*pPreprocessedMwcs, ccType, nrOfColors, nrOfIterations, rootNodeSet, noPreprocess);
            //ccadapter->solve(*pMwcs, ccType, nrOfColors, nrOfIterations, rootNodeSet);

            //ColorCodingBlockCutType *ccB = new ColorCodingBlockCutType();
            //ccB->init(*pPreprocessedMwcs, nrOfColors, nrOfIterations);
            /*
              if (rootNodeSet.size() > 0) {
                ColorCodingRootedType *ccR = new ColorCodingRootedType();
                //cc->init(*pPreprocessedMwcs, nrOfColors, nrOfIterations);
                ccR->init(*pPreprocessedMwcs, nrOfColors, nrOfIterations, rootNodeSet);
                //ColorCodingType::init(*pMwcs);
              } else {
                  ColorCodingUnrootedType *ccU = new ColorCodingUnrootedType();
                  //cc->init(*pPreprocessedMwcs, nrOfColors, nrOfIterations);
                  ccU->init(*pPreprocessedMwcs, nrOfColors, nrOfIterations);
                  //ColorCodingType::init(*pMwcs);
              }
              */
        } else {
