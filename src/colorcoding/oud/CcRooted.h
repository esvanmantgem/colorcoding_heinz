...
//
#ifndef HEINZ_CCROOTED_H
#define HEINZ_CCROOTED_H
...
namespace colcod {
    ...
    public:
        ...
        void update_table(Table table, std::vector<std::string> labels, std::vector<uint32_t> colorset, CcResult &result) {
            ...
            std::cout << "Max weight: " << best_weight << " with " << best_graph << std::endl;
        ...
        inline Table cc_iter(std::vector<double> nodes, std::vector<std::string> labels, std::vector<u_int32_t> colorset, int number_of_colors, std::vector<std::vector<int>> neighbors) {
        ...
        CcResult run(std::vector<double> nodes, std::vector<std::string> labels, std::vector<std::vector<int>> neighbors,
                     int number_of_colors, int number_of_iterations, std::vector<std::string> rootNodeSet) {
            ...
            for (int iter = 0; iter < number_of_iterations; iter++) {
                ...
                Table table = cc_iter(nodes, labels, colorset, number_of_colors, neighbors);
                // verander naam
                update_table(table, labels, colorset, result);
            }
            return result;
        }
    };
