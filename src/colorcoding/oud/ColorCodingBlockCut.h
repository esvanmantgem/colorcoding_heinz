//
// Created by ishuena on 03.05.18.
//

#ifndef HEINZ_COLORCODINGBLOCKCUT_H
#define HEINZ_COLORCODINGBLOCKCUT_H
...
namespace colcod {
    ...
    public:
        ...
        bool contains_roots(std::string path) {
        ...
        void create_colors(std::vector<u_int32_t> *node_colors, int nodes, int k, std::vector<std::string> labels) {
            ...
            for (int i = 0; i < nodes; i++) {
                ...
                if (std::find(rootSet.begin(), rootSet.end(), labels[i]) == rootSet.end()) {
                    ...
                    int random = rooted ? (rand() % (k - rootSet.size())) + 1 : (rand() % k) + 1;
        ...
        void init_vertices(MwcsGraphType &parser, NodeSet set, std::vector<double> *nodes, std::vector<std::string> *labels,
                           std::vector<std::vector<int>> *neighbors, Node root) {

            const Graph &g = parser.getGraph();

            //for (auto rootnode : rootNodeSet) {
            if (root != lemon::INVALID) {
                rootSet.push_back(parser.getLabel(root));
            ...
            for (auto n : set) {
                ...
                for (IncEdgeIt edgeIt(g, n); edgeIt != lemon::INVALID; ++edgeIt) {
                    ...
                    // add egde only when opposite is in set as well
                    if (std::find(set.begin(), set.end(), g.oppositeNode(n, edgeIt)) != set.end()) {
    ...
    for (BcTreeBlockNodeIt v(T); v != lemon::INVALID; ++v) {
         * 1. Run unrooted, story only iff unrooted > rooted
         * 2. Run rooted
std::cout << "Numblock: ------------------- " << numblock << std::endl;
NodeSet nodes = bcTree.getRealNodes(v);
for (auto n : nodes) {
    std::cout << parser.getLabel(n) << std::endl;
}
numblock++;
}

        //std::cout << bcTree.getNumBlockTreeNodes() << std::endl;
        //std::cout << nComponents << std::endl;


         */

        void run_cc(MwcsPreGraphType &parser, Node root, const NodeSet &set, int number_of_iterations, int number_of_colors) {
            // init datastructure
            std::vector<u_int32_t> colorset;
            std::vector<double> nodes;
            std::vector<std::string> labels;
            std::vector<std::vector<int>> neighbors;
            int number_of_nodes = set.size();

            colorset.reserve(pow(2, k) - 1);
            nodes.reserve(number_of_nodes);
            labels.reserve(number_of_nodes);
            neighbors.reserve(number_of_nodes);

            init_colorset(&colorset);
            init_vertices(parser, set, &nodes, &labels, &neighbors, root);
            ...
            for (int iter = 0; iter < number_of_iterations; iter++) {
                ...
                node_colors.reserve(number_of_nodes);
                Table table;
                table.reserve(number_of_nodes);
        ...
        void init(MwcsPreGraphType &parser, int original_colors, int number_of_iterations) {
            std::cout << "BLOCK CUT VERSION OF COLOR CODING \n";
            //best_graph = "";
            ...
            if (nBlocks != 0) {
                for (int blockDegree = nBlocks > 1 ? 1 : 0; blockDegree >= 0; --blockDegree) {
                    ...
                    for (auto b : leaves) {
                        ...
                        // Doe stuff hier
                        std::cout << "nodeblock size: " << nodesInBlock.size() << std::endl;

                        // init settings for new cc
                        int number_of_colors = (nodesInBlock.size() < original_colors) ? nodesInBlock.size() : original_colors;
                        k = subset_start = number_of_colors;
                        best_weight = -DBL_MAX;
                        basic_colors = 0;
                        //run_cc(parser, orgC, nodesInBlock, number_of_iterations, number_of_colors);
            ...
            std::cout << "End of prog bluckkcut, kopm ie hier?" << std::endl;
...
#endif //HEINZ_COLORCODINGBLOCKCUT_H
